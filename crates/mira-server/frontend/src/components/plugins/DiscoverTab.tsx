...
export function DiscoverTab({
          ...
          <div className="mt-2 flex w-full max-w-md flex-col gap-2">
            {data.suggested_marketplaces.map((s) => (
              <button
                ...
                className="flex items-center gap-3 rounded-lg border border-border/70 bg-card/60 px-3 py-2.5 text-left transition-colors hover:border-border hover:bg-card disabled:opacity-60"
              >
                ...
                <span className="shrink-0 text-[12px] text-mira-purple">
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        {data.marketplaces.length > 1 && (
          <select
            ...
            className="ml-auto h-7 rounded-md border border-border bg-transparent px-2 text-[12px] text-muted-foreground"
      ...
      {shown.length === 0 ? (
        <div className="py-10 text-center text-[13px] text-muted-foreground">No plugins match.</div>
      ) : (
...
function Chip({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'h-7 rounded-full border px-3 text-[12px] capitalize transition-colors',
        active
...
function PluginCard({
  ...
  return (
    <div
      ...
      className="group flex cursor-pointer gap-3 rounded-xl border border-border/70 bg-card/50 p-3.5 text-left transition-colors hover:border-border hover:bg-card"
    >
      <Avatar name={entry.id} />
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-start gap-2">
          <div className="min-w-0 flex-1">
            <div className="truncate text-[14px] font-medium">{entry.display_name || entry.name}</div>
            <div className="truncate text-[11.5px] text-muted-foreground">
              {[entry.author, showMarket ? entry.marketplace : null].filter(Boolean).join(' · ') || entry.marketplace}
            </div>
          </div>
          {entry.installed ? (
            <Pill tone={entry.enabled ? 'green' : 'neutral'} className="mt-0.5">
              {entry.enabled ? (
                <>
