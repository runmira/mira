    ...
    async fn invoke(&self, call: &ToolCall, ctx: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        let (final_cmd, fell_back) = if crate::builtin::rg::rg_available() {
            ...
            return Err(ToolError::Failed(
                "ripgrep (`rg`) is not installed and multi-pattern find_symbol \
                 can't be translated to POSIX grep. Install ripgrep to \
                 continue (`brew install ripgrep` or `cargo install ripgrep`)."
                    .to_string(),
            ));
        };
